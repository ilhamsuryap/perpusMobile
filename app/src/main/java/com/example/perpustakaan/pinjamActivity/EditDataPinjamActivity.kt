        ...
        binding.btnHapus.setOnClickListener {
            if (pinjamId != -1) {
                lifecycleScope.launch(Dispatchers.IO) {
                    try {
                        val pinjamDao = PerpustakaanDatabase.getDatabase(this@EditDataPinjamActivity).daopinjam()
                        pinjamDao.deletePinjamById(pinjamId)

                        launch(Dispatchers.Main) {
                            Toast.makeText(this@EditDataPinjamActivity, "Buku berhasil dihapus", Toast.LENGTH_SHORT).show()
                            finish() // Kembali ke layar sebelumnya
                        }
                    } catch (e: Exception) {
                        launch(Dispatchers.Main) {
                            Toast.makeText(this@EditDataPinjamActivity, "Gagal menghapus Buku", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
